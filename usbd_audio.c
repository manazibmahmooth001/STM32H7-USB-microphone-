/**
  ...
  *             - 1 Audio Streaming Interface (with 7 channels, PCM, microphone input mode)
  *             - 1 Audio Streaming Endpoint
  *             - 1 Audio Terminal Input (7 channels)
  ...
  *             - sampling rate: 48KHz.
  *             - Bit resolution: 16
  *             - Number of channels: 7 (microphone array)
  *             - No volume control
...
#define AUDIO_PACKET_SZE(frq) \
  (uint8_t)(((frq * 2U * 7U) / 1000U) & 0xFFU), (uint8_t)((((frq * 2U * 7U) / 1000U) >> 8) & 0xFFU)

#ifdef USE_USBD_COMPOSITE
#define AUDIO_PACKET_SZE_WORD(frq)     (uint32_t)((((frq) * 2U * 7U)/1000U))
...
#ifndef USE_USBD_COMPOSITE
/* USB AUDIO device Configuration Descriptor for 7-channel microphone input */
__ALIGN_BEGIN static uint8_t USBD_AUDIO_CfgDesc[USB_AUDIO_CONFIG_DESC_SIZ] __ALIGN_END =
{
  ...
  0x01,                                 /* bTerminalID */
  0x02,                                 /* wTerminalType AUDIO_TERMINAL_MICROPHONE   0x0202 */
  0x02,
  0x00,                                 /* bAssocTerminal */
  0x07,                                 /* bNrChannels = 7 for microphone array */
  0x7F,                                 /* wChannelConfig 0x007F for 7 discrete channels */
  0x00,
  0x00,                                 /* iChannelNames */
  0x00,                                 /* iTerminal */
  /* 12 byte*/
  ...
  /* Endpoint 1 - Standard Descriptor for IN endpoint (microphone input to host) */
  AUDIO_STANDARD_ENDPOINT_DESC_SIZE,    /* bLength */
  USB_DESC_TYPE_ENDPOINT,               /* bDescriptorType */
  AUDIO_IN_EP,                          /* bEndpointAddress: 0x81 = IN endpoint 1 */
  USBD_EP_TYPE_ISOC,                    /* bmAttributes */
  AUDIO_PACKET_SZE(USBD_AUDIO_FREQ),    /* wMaxPacketSize in Bytes (Freq(Samples)*2(bytes)*7(channels)) */
...
static uint8_t USBD_AUDIO_Init(USBD_HandleTypeDef *pdev, uint8_t cfgidx)
{
  ...
  /* Open EP IN for microphone input */
        ...
        case USB_REQ_SET_INTERFACE:
          if (pdev->dev_state == USBD_STATE_CONFIGURED)
          {
            if ((uint8_t)(req->wValue) <= USBD_MAX_NUM_INTERFACES)
            {
              ...
              /* Start microphone capture when interface is set to 1 */
              if (haudio->alt_setting == 1U)
              {
                /* Start audio capture */
                ((USBD_AUDIO_ItfTypeDef *)pdev->pUserData[pdev->classId])->AudioCmd(haudio->buffer,
                                                                                    AUDIO_OUT_PACKET,
                                                                                    AUDIO_CMD_START);
              }
              else
...
static uint8_t USBD_AUDIO_DataIn(USBD_HandleTypeDef *pdev, uint8_t epnum)
{
  ...
  {
    /* Handle microphone data transmission complete */
    ((USBD_AUDIO_ItfTypeDef *)pdev->pUserData[pdev->classId])->PeriodicTC(&haudio->buffer[haudio->rd_ptr],
                                                                          AUDIO_OUT_PACKET, AUDIO_IN_TC);
    ...
    if (haudio->alt_setting == 1U)
    {
      (void)USBD_LL_Transmit(pdev, AUDIOInEpAdd, &haudio->buffer[haudio->rd_ptr], AUDIO_OUT_PACKET);
    }
  }
...
static uint8_t USBD_AUDIO_EP0_TxReady(USBD_HandleTypeDef *pdev)
{
  ...
  /* Only IN control data are processed */
  return (uint8_t)USBD_OK;
}
/**
...
static uint8_t USBD_AUDIO_SOF(USBD_HandleTypeDef *pdev)
{
  ...
  if (haudio->alt_setting == 1U)
  {
    /* Trigger microphone data capture and transmission */
    ((USBD_AUDIO_ItfTypeDef *)pdev->pUserData[pdev->classId])->AudioCmd(&haudio->buffer[haudio->wr_ptr],
                                                                        AUDIO_OUT_PACKET,
                                                                        AUDIO_CMD_PLAY);
...
void USBD_AUDIO_Sync(USBD_HandleTypeDef *pdev, AUDIO_OffsetTypeDef offset)
{
  ...
  /* Microphone input synchronization */
...
static uint8_t USBD_AUDIO_IsoINIncomplete(USBD_HandleTypeDef *pdev, uint8_t epnum)
{
  ...
  /* Prepare IN endpoint to send next audio packet */
  if (haudio->alt_setting == 1U)
  {
    (void)USBD_LL_Transmit(pdev, epnum, &haudio->buffer[haudio->rd_ptr], AUDIO_OUT_PACKET);
  }
...
static uint8_t USBD_AUDIO_DataOut(USBD_HandleTypeDef *pdev, uint8_t epnum)
{
  ...
  /* Not used for microphone input - only IN data are processed */
  return (uint8_t)USBD_OK;
}
...
uint32_t USBD_AUDIO_GetEpPcktSze(USBD_HandleTypeDef *pdev, uint8_t If, uint8_t Ep)
{
  ...
  /* Return the wMaxPacketSize value in Bytes (Freq(Samples)*2(bytes)*7(channels)) */
  return mps;
}
#endif /* USE_USBD_COMPOSITE */
